...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] pt-24 text-stone-900 font-sans selection:bg-stone-900 selection:text-white">
      <Navbar />

      {/* --- EDITORIAL HERO SECTION --- */}
      <section className="pt-16 pb-16 px-6 lg:px-8 max-w-7xl mx-auto flex flex-col items-center text-center">
        <h4 className="text-stone-500 tracking-[0.3em] uppercase text-xs font-semibold mb-6">
          Advanced Dermatology in Nepal
        </h4>
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-serif font-medium leading-tight text-stone-900 mb-8 max-w-4xl">
          Healthy Skin, <br className="hidden md:block" />
          <span className="italic text-stone-500">Confident You.</span>
        </h1>
        <p className="text-lg md:text-xl text-stone-600 max-w-2xl mb-12 font-light">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <button
            ...
            className="px-10 py-4 bg-stone-900 text-white font-medium rounded-full hover:bg-stone-800 transition-colors"
          >
            Book Consultation
          </button>
          ...
          <div className="flex items-center gap-4">
            <a href="tel:+9779860831099" className="w-12 h-12 flex items-center justify-center rounded-full border border-stone-300 text-stone-700 hover:bg-stone-100 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
            </a>
            <a href="https://wa.me/9779860831099" target="_blank" rel="noopener noreferrer" className="w-12 h-12 flex items-center justify-center rounded-full border border-stone-300 text-stone-700 hover:bg-green-50 hover:text-green-600 hover:border-green-200 transition-colors">
          ...
          {isAuthenticated && isAdmin && (
            <button onClick={() => navigate("/admin")} className="px-6 py-4 border-2 border-stone-900 text-stone-900 font-medium rounded-full hover:bg-stone-100 transition-colors">
              Admin Panel
            </button>
          )}
        </div>
      ...
      <section className="max-w-[96%] mx-auto relative mb-24">
        ...
        {/* Changed background to a dark slate matching your screenshot */}
        <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-[90%] max-w-5xl bg-slate-900 p-8 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] grid grid-cols-2 md:grid-cols-4 gap-8 z-10">
          {[
            ...
            <div key={i} className="text-center">
              {/* Changed from text-stone-900 to text-white */}
              <h3 className="text-3xl md:text-4xl font-serif text-white mb-2">{stat.value}</h3>
              {/* Changed from text-stone-500 to text-slate-300 */}
      ...
      <section className="py-24 px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 border-b border-stone-200 pb-8">
          <div className="max-w-2xl">
            <h2 className="text-4xl md:text-5xl font-serif text-stone-900 mb-4">Curated Treatments</h2>
            <p className="text-stone-600 font-light">Comprehensive solutions tailored to your unique skin biology.</p>
          </div>
          <button onClick={() => navigate("/services")} className="mt-6 md:mt-0 pb-1 border-b border-stone-900 text-stone-900 font-medium hover:text-stone-500 hover:border-stone-500 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {[
            ...
            <div key={i} onClick={() => navigate("/services")} className="group cursor-pointer">
              <div className="aspect-square rounded-2xl mb-6 overflow-hidden relative bg-stone-100">
                <img 
                  src={service.image} 
                  alt={service.title} 
                ...
                <div className="absolute top-4 left-4 w-12 h-12 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm">
                  <span className="text-lg font-serif text-stone-900">0{i+1}</span>
                </div>
              </div>
              
              <h3 className="text-2xl font-serif text-stone-900 mb-3 group-hover:text-stone-600 transition-colors">{service.title}</h3>
              <p className="text-stone-500 font-light leading-relaxed">{service.desc}</p>
            </div>
          ))}
        </div>
      </section>
